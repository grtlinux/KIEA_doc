...
public class RP_MUR_WIP_SUMM1_MacroJob extends AbstractMacroJob
{
	...
	private RP_MUR_WIP_SUMM1_CsvIo writer1 = null;
	...
	private List<RP_MUR_WIP_SUMM1_Bean> outList1 = null;
	
	private String filePath = null;

	private String jobId = null;

	public RP_MUR_WIP_SUMM1_MacroJob(String jobId)
	{
		this.jobId = jobId;
	...
	public void beforeMacroJob()
	{
		...
		if (Flag.TRUE) {
			 ...
			 * 시간기록 시작
			 */
			RP_MUR_WIP_SUMM1_Main.runMSec = System.nanoTime();
		}
		...
		if (Flag.TRUE) {
			...
			bean.setWhere(String.format("JOB_ID = %s AND PROG_NM = %s", StrUtil.quote(jobId), StrUtil.quote(RP_MUR_WIP_SUMM1_Main.dsName)));
		...
		if (Flag.TRUE) {
			...
			bean.setFields(new String[][] {
					...
					{ "CMD_CODE   ", StrUtil.quote(RP_MUR_WIP_SUMM1_Main.dsName.substring(0, 3)) },
					{ "PROG_NM    ", StrUtil.quote(RP_MUR_WIP_SUMM1_Main.dsName) },
					{ "CSV_NM     ", StrUtil.quote(RP_MUR_WIP_SUMM1_Main.dsName + ".csv") },
		...
		if (Flag.TRUE) {
			try {
				...
				writer1 = new RP_MUR_WIP_SUMM1_CsvIo(this.filePath);
	...
	public void macroJob()
	{
		...
		if (Flag.TRUE) {
			try {
				...
				Map<String, RP_MUR_WIP_SUMM1_Bean> mapWipCluster = new LinkedHashMap<String, RP_MUR_WIP_SUMM1_Bean>();
				Map<String, List<SO_SYS_GLASS_LIST2_Bean>> mapListGlass = new LinkedHashMap<String, List<SO_SYS_GLASS_LIST2_Bean>>();
				Map<String, RP_MUR_WIP_SUMM1_Bean> mapWip = new LinkedHashMap<String, RP_MUR_WIP_SUMM1_Bean>();
				...
				if (Flag.TRUE) {
					...
					for (Map.Entry<String, SO_SYS_GLASS_LIST2_Bean> entry : mapGlass.entrySet()) {
						...
						RP_MUR_WIP_SUMM1_Bean totBean = mapWipCluster.get(keyCluster);
						if (totBean == null) {
							totBean = new RP_MUR_WIP_SUMM1_Bean();
					...
					if (!Flag.TRUE) {
						 ...
						 * 확인출력
						 */
						for (Map.Entry<String, RP_MUR_WIP_SUMM1_Bean> entry : mapWipCluster.entrySet()) {
							Print.println("[%s] -> %s", entry.getKey(), entry.getValue());
						}
				...
				if (Flag.TRUE) {
					...
					for (WIP_EQP_SMMRY_Bean inBean2 : inList2) {
						...
						for (SO_SYS_GLASS_LIST2_Bean inBean1 : listGlass) {
							...
							RP_MUR_WIP_SUMM1_Bean bean = mapWip.get(keyCnt);
							if (bean == null) {
								bean = new RP_MUR_WIP_SUMM1_Bean();
					...
					if (!Flag.TRUE) {
						 ...
						 * 확인출력
						 */
						for (Map.Entry<String, RP_MUR_WIP_SUMM1_Bean> entry : mapWip.entrySet()) {
				...
				if (Flag.TRUE) {
					...
					for (Map.Entry<String, RP_MUR_WIP_SUMM1_Bean> entry : mapWip.entrySet()) {
						...
						RP_MUR_WIP_SUMM1_Bean bean = entry.getValue();
					 ...
					 */
					for (RP_MUR_WIP_SUMM1_Bean outBean1 : outList1) {
						
						//RP_SYS_WIP_SUMM1_Bean bean = mapWip.get(outBean1.getClusterId() + ":" + outBean1.getStepId());
						RP_MUR_WIP_SUMM1_Bean bean = mapWipCluster.get(outBean1.getClusterId());
					...
					if (!Flag.TRUE) {
						 ...
						 * 확인출력
						 */
						for (RP_MUR_WIP_SUMM1_Bean bean : outList1) {
							System.out.println(bean);
						}
					}
				}
				...
				if (Flag.TRUE) {
					 ...
					 */
					Collections.sort(outList1, new Comparator<RP_MUR_WIP_SUMM1_Bean>() {
						@Override
						public int compare(RP_MUR_WIP_SUMM1_Bean bean1, RP_MUR_WIP_SUMM1_Bean bean2) {
							int ret = 0;
	...
	public void afterMacroJob()
	{
		...
		if (Flag.TRUE) {
			try {
				...
				RP_MUR_WIP_SUMM1_Main.cntWList = outList1.size();
		...
		if (Flag.TRUE) {
			 ...
			 * 시간기록 끝
			 */
			RP_MUR_WIP_SUMM1_Main.runMSec = (System.nanoTime() - RP_MUR_WIP_SUMM1_Main.runMSec) / 1000 / 1000;
			
			if (Flag.TRUE) Logger.info("##### took time : %d ms", RP_MUR_WIP_SUMM1_Main.runMSec);
		}
		...
		if (Flag.TRUE) {
			...
			bean.setTable("ANAL_CSV");
			bean.setFields(new String[][] {
					{ "LIST_CNT   ", "" + RP_MUR_WIP_SUMM1_Main.cntWList },
					{ "E_TIME     ", "SYSDATE" },
					{ "R_MSEC     ", "" + RP_MUR_WIP_SUMM1_Main.runMSec },
