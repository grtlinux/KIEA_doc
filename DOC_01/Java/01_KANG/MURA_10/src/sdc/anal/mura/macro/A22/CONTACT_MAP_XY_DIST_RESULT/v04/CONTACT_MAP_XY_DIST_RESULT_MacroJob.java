...
public class CONTACT_MAP_XY_DIST_RESULT_MacroJob extends AbstractMacroJob
{
	...
	public CONTACT_MAP_XY_DIST_RESULT_MacroJob(String jobId)
	{
		this.jobId = jobId;
	...
	public void beforeMacroJob()
	{
		...
		if (Flag.TRUE) {
			 ...
			 * 시간기록 시작
			 */
			CONTACT_MAP_XY_DIST_RESULT_Main.runMSec = System.nanoTime();
		}
		...
		if (Flag.TRUE) {
			...
			bean.setWhere(String.format("JOB_ID = %s AND PROG_NM = %s", StrUtil.quote(jobId), StrUtil.quote(CONTACT_MAP_XY_DIST_RESULT_Main.dsName)));
		...
		if (Flag.TRUE) {
			...
			bean.setFields(new String[][] {
					...
					{ "CMD_CODE   ", StrUtil.quote(CONTACT_MAP_XY_DIST_RESULT_Main.dsName.substring(0, 3)) },
					{ "PROG_NM    ", StrUtil.quote(CONTACT_MAP_XY_DIST_RESULT_Main.dsName) },
					{ "CSV_NM     ", StrUtil.quote(CONTACT_MAP_XY_DIST_RESULT_Main.dsName + ".csv") },
	...
	public void afterMacroJob()
	{
		...
		if (Flag.TRUE) {
			try {
				...
				CONTACT_MAP_XY_DIST_RESULT_Main.cntWList = outList1.size();
		...
		if (Flag.TRUE) {
			 ...
			 * 시간기록 끝
			 */
			CONTACT_MAP_XY_DIST_RESULT_Main.runMSec = (System.nanoTime() - CONTACT_MAP_XY_DIST_RESULT_Main.runMSec) / 1000 / 1000;
			
			if (Flag.TRUE) Logger.info("##### took time : %d ms", CONTACT_MAP_XY_DIST_RESULT_Main.runMSec);
		}
		...
		if (Flag.TRUE) {
			...
			bean.setTable("ANAL_CSV");
			bean.setFields(new String[][] {
					{ "LIST_CNT   ", "" + CONTACT_MAP_XY_DIST_RESULT_Main.cntWList },
					{ "E_TIME     ", "SYSDATE" },
					{ "R_MSEC     ", "" + CONTACT_MAP_XY_DIST_RESULT_Main.runMSec },
