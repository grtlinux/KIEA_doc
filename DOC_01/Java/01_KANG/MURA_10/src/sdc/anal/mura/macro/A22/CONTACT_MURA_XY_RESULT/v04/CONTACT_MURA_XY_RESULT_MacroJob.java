...
public class CONTACT_MURA_XY_RESULT_MacroJob extends AbstractMacroJob
{
	private CONTACT_MAP_XY_RESULT_CsvIo reader1 = null;
	private MURA_MAP_XY_RESULT_CsvIo reader2 = null;
	private CONTACT_MURA_XY_RESULT_CsvIo writer1 = null;
	
	private List<CONTACT_MAP_XY_RESULT_Bean> inList1  = null;
	private List<MURA_MAP_XY_RESULT_Bean> inList2  = null;
	private List<CONTACT_MURA_XY_RESULT_Bean> outList1 = null;
	
	private String filePath = null;

	private String jobId = null;

	public CONTACT_MURA_XY_RESULT_MacroJob(String jobId)
	{
		this.jobId = jobId;
	...
	public void beforeMacroJob()
	{
		...
		if (Flag.TRUE) {
			 ...
			 * 시간기록 시작
			 */
			CONTACT_MURA_XY_RESULT_Main.runMSec = System.nanoTime();
		}
		...
		if (Flag.TRUE) {
			...
			bean.setWhere(String.format("JOB_ID = %s AND PROG_NM = %s", StrUtil.quote(jobId), StrUtil.quote(CONTACT_MURA_XY_RESULT_Main.dsName)));
		...
		if (Flag.TRUE) {
			...
			bean.setFields(new String[][] {
					...
					{ "CMD_CODE   ", StrUtil.quote(CONTACT_MURA_XY_RESULT_Main.dsName.substring(0, 3)) },
					{ "PROG_NM    ", StrUtil.quote(CONTACT_MURA_XY_RESULT_Main.dsName) },
					{ "CSV_NM     ", StrUtil.quote(CONTACT_MURA_XY_RESULT_Main.dsName + ".csv") },
		...
		if (Flag.TRUE) {
			try {
				...
				writer1 = new CONTACT_MURA_XY_RESULT_CsvIo(this.filePath);
				...
				if (Flag.TRUE) {
					...
					for (CONTACT_MAP_XY_RESULT_Bean inBean1 : inList1) {
						...
						for (MURA_MAP_XY_RESULT_Bean inBean2 : inList2) {
							...
							double[] muraRect = getContactRect(coordRect, cellRect);
							if (muraRect == null)
								continue;
							
							CONTACT_MURA_XY_RESULT_Bean outBean1 = new CONTACT_MURA_XY_RESULT_Bean();
							...
							outBean1.setMuraX1         ("" + muraRect[0]);
							outBean1.setMuraY1         ("" + muraRect[1]);
							outBean1.setMuraX2         ("" + muraRect[2]);
							outBean1.setMuraY2         ("" + muraRect[3]);
	...
	public void afterMacroJob()
	{
		...
		if (Flag.TRUE) {
			try {
				...
				CONTACT_MURA_XY_RESULT_Main.cntWList = outList1.size();
		...
		if (Flag.TRUE) {
			 ...
			 * 시간기록 끝
			 */
			CONTACT_MURA_XY_RESULT_Main.runMSec = (System.nanoTime() - CONTACT_MURA_XY_RESULT_Main.runMSec) / 1000 / 1000;
			
			if (Flag.TRUE) Logger.info("##### took time : %d ms", CONTACT_MURA_XY_RESULT_Main.runMSec);
		}
		...
		if (Flag.TRUE) {
			...
			bean.setTable("ANAL_CSV");
			bean.setFields(new String[][] {
					{ "LIST_CNT   ", "" + CONTACT_MURA_XY_RESULT_Main.cntWList },
					{ "E_TIME     ", "SYSDATE" },
					{ "R_MSEC     ", "" + CONTACT_MURA_XY_RESULT_Main.runMSec },
