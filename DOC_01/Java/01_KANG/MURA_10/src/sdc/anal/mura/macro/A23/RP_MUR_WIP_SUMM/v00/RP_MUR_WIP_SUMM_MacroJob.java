...
public class RP_MUR_WIP_SUMM_MacroJob extends AbstractMacroJob
{
	...
	public RP_MUR_WIP_SUMM_MacroJob(String jobId)
	{
		this.jobId = jobId;
	...
	public void beforeMacroJob()
	{
		...
		if (Flag.TRUE) {
			try {
				...
				inList4  = reader4.getList(true);
				inList5  = reader5.getList(true);
	...
	public void macroJob()
	{
		...
		if (Flag.TRUE) {
			try {
				 ...
				 * Job
				 */
				Map<String, List<SO_SYS_GLASS_LIST2_Bean>> listMap = new LinkedHashMap<String, List<SO_SYS_GLASS_LIST2_Bean>>();
				...
				if (Flag.TRUE) {
					for (SO_SYS_GLASS_LIST2_Bean inBean1 : inList1) {
						...
						List<SO_SYS_GLASS_LIST2_Bean> list = listMap.get(key);
						if (list == null) {
					...
					if (!Flag.TRUE) {
						 ...
						 * 확인
						 */
						for (Map.Entry<String, List<SO_SYS_GLASS_LIST2_Bean>> entry : listMap.entrySet()) {
							System.out.println("[" + entry.getKey() + "] =>");
							for (SO_SYS_GLASS_LIST2_Bean inBean1 : entry.getValue()) {
								System.out.println("\t\t" + inBean1);
							}
						}
					}
				}
				...
				if (Flag.TRUE) {
					for (WIP_EQP_SMMRY_Bean inBean2 : inList2) {
						...
						for (SO_SYS_GLASS_LIST2_Bean inBean1 : list) {
					...
					for (GLASS_CELL_Bean inBean3 : inList3) {
						...
						if (Flag.TRUE) {
							...
							if (inBean4 == null) {
								...
								inBean4.setGrpBadCnt("" + 0);
								inBean4.setGrpGoodCnt("" + 0);
							...
							if ("1".equals(inBean3.getTarget())) {
								inBean4.addGrpBadCnt();
							} else {
								inBean4.addGrpGoodCnt();
							}
						}
				...
				if (Flag.TRUE) {
					...
					for (Map.Entry<String, EQP_CNT_Bean> entry : eqpCntMap.entrySet()) {
						...
						int grpBadCnt = Integer.parseInt(groupCntMap.get(inBean5.getGroupNum()).getGrpBadCnt());
						int grpGoodCnt = Integer.parseInt(groupCntMap.get(inBean5.getGroupNum()).getGrpGoodCnt());
						
						double badRatio = badCnt / grpBadCnt;
						double goodRatio = goodCnt / grpGoodCnt;
						...
						outBean1.setGrpBadCnt ("" + grpBadCnt );
						outBean1.setGrpGoodCnt("" + grpGoodCnt);
				...
				if (Flag.TRUE) {
					...
					Collections.sort(outList1, new Comparator<RP_MUR_WIP_SUMM_Bean>() {
						...
						public int compare(RP_MUR_WIP_SUMM_Bean bean1, RP_MUR_WIP_SUMM_Bean bean2) {
							double d1 = Double.parseDouble(bean1.getBadRatio());
							double d2 = Double.parseDouble(bean2.getBadRatio());
					...
					int rank = 0;
					for (RP_MUR_WIP_SUMM_Bean bean : outList1) {
						rank++;
						bean.setRankBadRatio("" + rank);
					}
				}
				...
				if (Flag.TRUE) {
					...
					for (RP_MUR_WIP_SUMM_Bean bean : outList1) {
						rank++;
						bean.setRankBGRatio("" + rank);
					}
				}
				
				if (Flag.TRUE) {
					/*
					 * SORT : GroupNum, StepId, EqpId
					 */
					Collections.sort(outList1, new Comparator<RP_MUR_WIP_SUMM_Bean>() {
						@Override
						public int compare(RP_MUR_WIP_SUMM_Bean bean1, RP_MUR_WIP_SUMM_Bean bean2) {
