package sdc.anal.mura.macro.A23.RP_MUR_WIP_SUMM4.v04;
	...
	public void macroJob()
	{
		...
		if (Flag.TRUE) {
			try {
				...
				Map<String, RP_MUR_WIP_SUMM4_Bean> mapWipCluster = new LinkedHashMap<String, RP_MUR_WIP_SUMM4_Bean>();
				...
				if (Flag.TRUE) {
					...
					for (Map.Entry<String, SO_SYS_GLASS_LIST2_Bean> entry : mapGlass.entrySet()) {
						...
						RP_MUR_WIP_SUMM4_Bean totBean = mapWipCluster.get(keyCluster);
						if (totBean == null) {
							totBean = new RP_MUR_WIP_SUMM4_Bean();
					...
					if (!Flag.TRUE) {
						 ...
						 * 확인출력
						 */
						for (Map.Entry<String, RP_MUR_WIP_SUMM4_Bean> entry : mapWipCluster.entrySet()) {
							Print.println("[%s] -> %s", entry.getKey(), entry.getValue());
						}
				...
				if (Flag.TRUE) {
					...
					for (WIP_EQP_SMMRY_Bean inBean2 : inList2) {
						...
						for (SO_SYS_GLASS_LIST2_Bean inBean1 : listGlass) {
							...
							RP_MUR_WIP_SUMM4_Bean bean = mapWip.get(keyCnt);
							if (bean == null) {
								bean = new RP_MUR_WIP_SUMM4_Bean();
				...
				if (Flag.TRUE) {
					...
					for (Map.Entry<String, RP_MUR_WIP_SUMM4_Bean> entry : mapWip.entrySet()) {
						...
						RP_MUR_WIP_SUMM4_Bean bean = entry.getValue();
					...
					for (RP_MUR_WIP_SUMM4_Bean outBean1 : outList1) {
						...
						RP_MUR_WIP_SUMM4_Bean bean = mapWipCluster.get(outBean1.getClusterId());
					...
					if (!Flag.TRUE) {
						 ...
						 * 확인출력
						 */
						for (RP_MUR_WIP_SUMM4_Bean bean : outList1) {
							System.out.println(bean);
						}
					}
				}
				...
				if (Flag.TRUE) {
					...
					for (int i=outList1.size()-1; 0 <= i; i--) {
						double bRatio = Double.parseDouble(outList1.get(i).getBadRatio());
						if (80.0 > bRatio) {
							outList1.remove(i);
						}
					}
