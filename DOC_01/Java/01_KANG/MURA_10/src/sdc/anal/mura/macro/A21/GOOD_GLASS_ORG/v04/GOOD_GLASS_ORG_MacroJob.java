...
public class GOOD_GLASS_ORG_MacroJob extends AbstractMacroJob
{
	...
	public void beforeMacroJob()
	{
		...
		if (Flag.TRUE) {
			 ...
			 * 시간기록 시작
			 */
			GOOD_GLASS_ORG_Main.runMSec = System.nanoTime();
		}
		...
		if (Flag.TRUE) {
			...
			bean.setWhere(String.format("JOB_ID = %s AND PROG_NM = %s", StrUtil.quote(jobId), StrUtil.quote(GOOD_GLASS_ORG_Main.dsName)));
		...
		if (Flag.TRUE) {
			...
			bean.setFields(new String[][] {
					...
					{ "CMD_CODE   ", StrUtil.quote(GOOD_GLASS_ORG_Main.dsName.substring(0, 3)) },
					{ "PROG_NM    ", StrUtil.quote(GOOD_GLASS_ORG_Main.dsName) },
					{ "CSV_NM     ", StrUtil.quote(GOOD_GLASS_ORG_Main.dsName + ".csv") },
	...
	public void afterMacroJob()
	{
		...
		if (Flag.TRUE) {
			try {
				...
				GOOD_GLASS_ORG_Main.cntWList = outList1.size();
		...
		if (Flag.TRUE) {
			 ...
			 * 시간기록 끝
			 */
			GOOD_GLASS_ORG_Main.runMSec = (System.nanoTime() - GOOD_GLASS_ORG_Main.runMSec) / 1000 / 1000;
			
			if (Flag.TRUE) Logger.info("##### took time : %d ms", GOOD_GLASS_ORG_Main.runMSec);
		}
		...
		if (Flag.TRUE) {
			...
			bean.setTable("ANAL_CSV");
			bean.setFields(new String[][] {
					{ "LIST_CNT   ", "" + GOOD_GLASS_ORG_Main.cntWList },
					{ "E_TIME     ", "SYSDATE" },
					{ "R_MSEC     ", "" + GOOD_GLASS_ORG_Main.runMSec },
