...
public class RP_MUR_ERD_FDC_SUM01_MacroJob extends AbstractMacroJob
{
	private SO_SYS_GLASS_LIST2_CsvIo reader1 = null;
	private RP_MUR_WIP_SUMM1_CsvIo reader2 = null;
	private RP_MUR_ERD_FDC_SUM01_OracleReader reader3 = null;
	private RP_MUR_ERD_FDC_SUM01_CsvIo writer1 = null;
	
	private List<SO_SYS_GLASS_LIST2_Bean> inList1  = null;
	private List<RP_MUR_WIP_SUMM1_Bean> inList2  = null;
	private List<RP_MUR_ERD_FDC_SUM01_Bean> inList3 = null;
	private List<RP_MUR_ERD_FDC_SUM01_Bean> outList1 = null;
	
	private String filePath = null;

	private String jobId = null;

	public RP_MUR_ERD_FDC_SUM01_MacroJob(String jobId)
	{
		this.jobId = jobId;
	...
	public void beforeMacroJob()
	{
		...
		if (Flag.TRUE) {
			 ...
			 * 시간기록 시작
			 */
			RP_MUR_ERD_FDC_SUM01_Main.runMSec = System.nanoTime();
		}
		...
		if (Flag.TRUE) {
			...
			bean.setWhere(String.format("JOB_ID = %s AND PROG_NM = %s", StrUtil.quote(jobId), StrUtil.quote(RP_MUR_ERD_FDC_SUM01_Main.dsName)));
		...
		if (Flag.TRUE) {
			...
			bean.setFields(new String[][] {
					...
					{ "CMD_CODE   ", StrUtil.quote(RP_MUR_ERD_FDC_SUM01_Main.dsName.substring(0, 3)) },
					{ "PROG_NM    ", StrUtil.quote(RP_MUR_ERD_FDC_SUM01_Main.dsName) },
					{ "CSV_NM     ", StrUtil.quote(RP_MUR_ERD_FDC_SUM01_Main.dsName + ".csv") },
		...
		if (Flag.TRUE) {
			try {
				...
				reader2 = new RP_MUR_WIP_SUMM1_CsvIo(this.filePath);
				reader3 = new RP_MUR_ERD_FDC_SUM01_OracleReader();
				writer1 = new RP_MUR_ERD_FDC_SUM01_CsvIo(this.filePath);
	...
	public void macroJob()
	{
		...
		if (Flag.TRUE) {
			try {
				...
				if (Flag.TRUE) {
					for (RP_MUR_ERD_FDC_SUM01_Bean readBean : inList3) {
						RP_MUR_ERD_FDC_SUM01_Bean writeBean = new RP_MUR_ERD_FDC_SUM01_Bean();
	...
	public void afterMacroJob()
	{
		...
		if (Flag.TRUE) {
			try {
				...
				RP_MUR_ERD_FDC_SUM01_Main.cntWList = outList1.size();
		...
		if (Flag.TRUE) {
			 ...
			 * 시간기록 끝
			 */
			RP_MUR_ERD_FDC_SUM01_Main.runMSec = (System.nanoTime() - RP_MUR_ERD_FDC_SUM01_Main.runMSec) / 1000 / 1000;
			
			if (Flag.TRUE) Logger.info("##### took time : %d ms", RP_MUR_ERD_FDC_SUM01_Main.runMSec);
		}
		...
		if (Flag.TRUE) {
			...
			bean.setTable("ANAL_CSV");
			bean.setFields(new String[][] {
					{ "LIST_CNT   ", "" + RP_MUR_ERD_FDC_SUM01_Main.cntWList },
					{ "E_TIME     ", "SYSDATE" },
					{ "R_MSEC     ", "" + RP_MUR_ERD_FDC_SUM01_Main.runMSec },
