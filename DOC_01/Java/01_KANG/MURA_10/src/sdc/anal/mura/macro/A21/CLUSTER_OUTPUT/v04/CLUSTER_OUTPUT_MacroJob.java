...
public class CLUSTER_OUTPUT_MacroJob extends AbstractMacroJob
{
	...
	public void beforeMacroJob()
	{
		...
		if (Flag.TRUE) {
			 ...
			 * 시간기록 시작
			 */
			CLUSTER_OUTPUT_Main.runMSec = System.nanoTime();
		}
		...
		if (Flag.TRUE) {
			...
			bean.setWhere(String.format("JOB_ID = %s AND PROG_NM = %s", StrUtil.quote(jobId), StrUtil.quote(CLUSTER_OUTPUT_Main.dsName)));
		...
		if (Flag.TRUE) {
			...
			bean.setFields(new String[][] {
					...
					{ "CMD_CODE   ", StrUtil.quote(CLUSTER_OUTPUT_Main.dsName.substring(0, 3)) },
					{ "PROG_NM    ", StrUtil.quote(CLUSTER_OUTPUT_Main.dsName) },
					{ "CSV_NM     ", StrUtil.quote(CLUSTER_OUTPUT_Main.dsName + ".csv") },
	...
	public void afterMacroJob()
	{
		...
		if (Flag.TRUE) {
			try {
				...
				CLUSTER_OUTPUT_Main.cntWList = outList1.size();
		...
		if (Flag.TRUE) {
			 ...
			 * 시간기록 끝
			 */
			CLUSTER_OUTPUT_Main.runMSec = (System.nanoTime() - CLUSTER_OUTPUT_Main.runMSec) / 1000 / 1000;
			
			if (Flag.TRUE) Logger.info("##### took time : %d ms", CLUSTER_OUTPUT_Main.runMSec);
		}
		...
		if (Flag.TRUE) {
			...
			bean.setTable("ANAL_CSV");
			bean.setFields(new String[][] {
					{ "LIST_CNT   ", "" + CLUSTER_OUTPUT_Main.cntWList },
					{ "E_TIME     ", "SYSDATE" },
					{ "R_MSEC     ", "" + CLUSTER_OUTPUT_Main.runMSec },
