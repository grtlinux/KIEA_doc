...
public class RP_SYS_WIP_SUMM3_MacroJob extends AbstractMacroJob
{
	...
	public void beforeMacroJob()
	{
		...
		if (Flag.TRUE) {
			 ...
			 * 시간기록 시작
			 */
			RP_SYS_WIP_SUMM3_Main.runMSec = System.nanoTime();
		}
		...
		if (Flag.TRUE) {
			...
			bean.setWhere(String.format("JOB_ID = %s AND PROG_NM = %s", StrUtil.quote(jobId), StrUtil.quote(RP_SYS_WIP_SUMM3_Main.dsName)));
		...
		if (Flag.TRUE) {
			...
			bean.setFields(new String[][] {
					...
					{ "CMD_CODE   ", StrUtil.quote(RP_SYS_WIP_SUMM3_Main.dsName.substring(0, 3)) },
					{ "PROG_NM    ", StrUtil.quote(RP_SYS_WIP_SUMM3_Main.dsName) },
					{ "CSV_NM     ", StrUtil.quote(RP_SYS_WIP_SUMM3_Main.dsName + ".csv") },
			...
			try {
				...
				if (Flag.TRUE) {
					...
					for (WIP_EQP_SMMRY_Bean inBean2 : inList2) {
						String key = inBean2.getGlassId() + ":" + inBean2.getStepId() + ":" + inBean2.getEqpId();
	...
	public void afterMacroJob()
	{
		...
		if (Flag.TRUE) {
			try {
				...
				RP_SYS_WIP_SUMM3_Main.cntWList = outList1.size();
		...
		if (Flag.TRUE) {
			 ...
			 * 시간기록 끝
			 */
			RP_SYS_WIP_SUMM3_Main.runMSec = (System.nanoTime() - RP_SYS_WIP_SUMM3_Main.runMSec) / 1000 / 1000;
			
			if (Flag.TRUE) Logger.info("##### took time : %d ms", RP_SYS_WIP_SUMM3_Main.runMSec);
		}
		...
		if (Flag.TRUE) {
			...
			bean.setTable("ANAL_CSV");
			bean.setFields(new String[][] {
					{ "LIST_CNT   ", "" + RP_SYS_WIP_SUMM3_Main.cntWList },
					{ "E_TIME     ", "SYSDATE" },
					{ "R_MSEC     ", "" + RP_SYS_WIP_SUMM3_Main.runMSec },
