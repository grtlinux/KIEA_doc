...
public class EQP_FDC_SUM05_MacroJob extends AbstractMacroJob
{
	private EQP_FDC_SUM04_CsvIo reader1 = null;
	private EQP_FDC_SUM05_CsvIo writer1 = null;
	
	private List<EQP_FDC_SUM04_Bean> inList1  = null;
	private List<EQP_FDC_SUM05_Bean> outList1 = null;
	
	private String filePath = null;

	private String jobId = null;

	public EQP_FDC_SUM05_MacroJob(String jobId)
	{
		this.jobId = jobId;
	...
	public void beforeMacroJob()
	{
		...
		if (Flag.TRUE) {
			 ...
			 * 시간기록 시작
			 */
			EQP_FDC_SUM05_Main.runMSec = System.nanoTime();
		}
		...
		if (Flag.TRUE) {
			...
			bean.setWhere(String.format("JOB_ID = %s AND PROG_NM = %s", StrUtil.quote(jobId), StrUtil.quote(EQP_FDC_SUM05_Main.dsName)));
		...
		if (Flag.TRUE) {
			...
			bean.setFields(new String[][] {
					...
					{ "CMD_CODE   ", StrUtil.quote(EQP_FDC_SUM05_Main.dsName.substring(0, 3)) },
					{ "PROG_NM    ", StrUtil.quote(EQP_FDC_SUM05_Main.dsName) },
					{ "CSV_NM     ", StrUtil.quote(EQP_FDC_SUM05_Main.dsName + ".csv") },
		...
		if (Flag.TRUE) {
			try {
				reader1 = new EQP_FDC_SUM04_CsvIo(this.filePath);
				writer1 = new EQP_FDC_SUM05_CsvIo(this.filePath);
		...
		if (Flag.TRUE) {
			try {
				...
				if (Flag.TRUE) {
					...
					List<EQP_FDC_SUM05_Bean> list = new ArrayList<EQP_FDC_SUM05_Bean>();

					for (EQP_FDC_SUM04_Bean inBean1 : inList1) {
						if (oldClusterId.equals(inBean1.getClusterId())
							...
							EQP_FDC_SUM05_Bean bean = new EQP_FDC_SUM05_Bean();
				...
				if (Flag.TRUE) {
					...
					Collections.sort(outList1, new Comparator<EQP_FDC_SUM05_Bean>() {
						@Override
						public int compare(EQP_FDC_SUM05_Bean bean1, EQP_FDC_SUM05_Bean bean2) {
							int ret = 0;
	...
	public void afterMacroJob()
	{
		...
		if (Flag.TRUE) {
			try {
				...
				EQP_FDC_SUM05_Main.cntWList = outList1.size();
		...
		if (Flag.TRUE) {
			 ...
			 * 시간기록 끝
			 */
			EQP_FDC_SUM05_Main.runMSec = (System.nanoTime() - EQP_FDC_SUM05_Main.runMSec) / 1000 / 1000;
			
			if (Flag.TRUE) Logger.info("##### took time : %d ms", EQP_FDC_SUM05_Main.runMSec);
		}
		...
		if (Flag.TRUE) {
			...
			bean.setTable("ANAL_CSV");
			bean.setFields(new String[][] {
					{ "LIST_CNT   ", "" + EQP_FDC_SUM05_Main.cntWList },
					{ "E_TIME     ", "SYSDATE" },
					{ "R_MSEC     ", "" + EQP_FDC_SUM05_Main.runMSec },
	...
	private void printDataMaxIG(List<EQP_FDC_SUM05_Bean> list)
	{
		if (Flag.TRUE) {
			if (list.size() < 2)
				return;
		...
		if (Flag.TRUE) {
			...
			Collections.sort(list, new Comparator<EQP_FDC_SUM05_Bean>() {
				@Override
				public int compare(EQP_FDC_SUM05_Bean bean1, EQP_FDC_SUM05_Bean bean2) {
					int ret = 0;
