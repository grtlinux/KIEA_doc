	...
	public void macroJob()
	{
		...
		if (Flag.TRUE) {
			try {
				...
				if (Flag.TRUE) {
					/*
					 * SO_SYS_CELLID_LIST2 : DISTINCT(clusterid + glassid)
					 */
				...
				if (Flag.TRUE) {
					...
					for (WIP_EQP_SMMRY_Bean inBean2 : inList2) {
						...
						for (SO_SYS_CELLID_LIST2_Bean inBean1 : listGlass) {
							...
							if (!Flag.TRUE) {
								...
								RP_SYS_WIP_SUMM1_Bean bean = mapWip.get(keyTotal);
								if (bean == null) {
									bean = new RP_SYS_WIP_SUMM1_Bean();
									
									bean.setClusterId (keyTotal);
									bean.setTotBadCnt ("0");
					...
					if (!Flag.TRUE) {
						/*
						 * DATE.2014.11.05 : TOTAL을 구한다.  -> 아래로 로직 수정됨
						 */
						for (Map.Entry<String, RP_SYS_WIP_SUMM1_Bean> entry : mapWip.entrySet()) {
							RP_SYS_WIP_SUMM1_Bean bean = entry.getValue();
							String keyCluster = bean.getClusterId();
							
							RP_SYS_WIP_SUMM1_Bean beanCluster = mapWipCluster.get(keyCluster);
							if (beanCluster == null) {
								beanCluster = new RP_SYS_WIP_SUMM1_Bean();
								
								beanCluster.setClusterId (keyCluster);
								beanCluster.setTotBadCnt ("0");
								beanCluster.setTotGoodCnt("0");
								
								mapWipCluster.put(keyCluster, beanCluster);
							}
							
							beanCluster.addTotBadCnt (bean.getBadCnt ());
							beanCluster.addTotGoodCnt(bean.getGoodCnt());
						}
					...
					if (Flag.TRUE) {
						...
						for (Map.Entry<String, SO_SYS_CELLID_LIST2_Bean> entry : mapGlass.entrySet()) {
							...
							if ("BAD".equals(inBean1.getGroup())) {
								beanCluster.addTotBadCnt ();
							} else {
								beanCluster.addTotGoodCnt();
							}
						}
