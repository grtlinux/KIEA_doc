...
public class RP_TOTWIP_SUMM_MacroJob extends AbstractMacroJob
{
	...
	public void beforeMacroJob()
	{
		...
		if (Flag.TRUE) {
			 ...
			 * 시간기록 시작
			 */
			RP_TOTWIP_SUMM_Main.runMSec = System.nanoTime();
		}
		...
		if (Flag.TRUE) {
			...
			bean.setWhere(String.format("JOB_ID = %s AND PROG_NM = %s", StrUtil.quote(jobId), StrUtil.quote(RP_TOTWIP_SUMM_Main.dsName)));
		...
		if (Flag.TRUE) {
			...
			bean.setFields(new String[][] {
					...
					{ "CMD_CODE   ", StrUtil.quote(RP_TOTWIP_SUMM_Main.dsName.substring(0, 3)) },
					{ "PROG_NM    ", StrUtil.quote(RP_TOTWIP_SUMM_Main.dsName) },
					{ "CSV_NM     ", StrUtil.quote(RP_TOTWIP_SUMM_Main.dsName + ".csv") },
		...
		if (Flag.TRUE) {
			try {
				...
				if (Flag.TRUE) {
					 ...
					 * -> 해당 GLASS 에 대한 CLUSTER_ID 목록을 만든다.
					 */
					for (Map.Entry<String, SO_SYS_CELLID_LIST2_Bean> entry : mapGlass.entrySet()) {
						SO_SYS_CELLID_LIST2_Bean inBean1 = entry.getValue();
				...
				if (Flag.TRUE) {
					/*
					 * TODO : 2014.11.06
					 * WIP_EQP_SMMRY(trckIn/OutHour) stepid + eqpid + glassid 중복제거 모듈 추가
					 */
	...
	public void afterMacroJob()
	{
		...
		if (Flag.TRUE) {
			try {
				...
				RP_TOTWIP_SUMM_Main.cntWList = outList1.size();
		...
		if (Flag.TRUE) {
			 ...
			 * 시간기록 끝
			 */
			RP_TOTWIP_SUMM_Main.runMSec = (System.nanoTime() - RP_TOTWIP_SUMM_Main.runMSec) / 1000 / 1000;
			
			if (Flag.TRUE) Logger.info("##### took time : %d ms", RP_TOTWIP_SUMM_Main.runMSec);
		}
		...
		if (Flag.TRUE) {
			...
			bean.setTable("ANAL_CSV");
			bean.setFields(new String[][] {
					{ "LIST_CNT   ", "" + RP_TOTWIP_SUMM_Main.cntWList },
					{ "E_TIME     ", "SYSDATE" },
					{ "R_MSEC     ", "" + RP_TOTWIP_SUMM_Main.runMSec },
