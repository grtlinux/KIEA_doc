...
public class RP_SYS_WIP_SUMM2_MacroJob extends AbstractMacroJob
{
	...
	public void beforeMacroJob()
	{
		...
		if (Flag.TRUE) {
			 ...
			 * 시간기록 시작
			 */
			RP_SYS_WIP_SUMM2_Main.runMSec = System.nanoTime();
		}
		...
		if (Flag.TRUE) {
			...
			bean.setWhere(String.format("JOB_ID = %s AND PROG_NM = %s", StrUtil.quote(jobId), StrUtil.quote(RP_SYS_WIP_SUMM2_Main.dsName)));
		...
		if (Flag.TRUE) {
			...
			bean.setFields(new String[][] {
					...
					{ "CMD_CODE   ", StrUtil.quote(RP_SYS_WIP_SUMM2_Main.dsName.substring(0, 3)) },
					{ "PROG_NM    ", StrUtil.quote(RP_SYS_WIP_SUMM2_Main.dsName) },
					{ "CSV_NM     ", StrUtil.quote(RP_SYS_WIP_SUMM2_Main.dsName + ".csv") },
	...
	public void macroJob()
	{
		...
		if (Flag.TRUE) {
			try {
				...
				Map<String, RP_SYS_WIP_SUMM2_Bean> mapWipCluster = new LinkedHashMap<String, RP_SYS_WIP_SUMM2_Bean>();
				...
				if (Flag.TRUE) {
					...
					for (Map.Entry<String, SO_SYS_CELLID_LIST2_Bean> entry : mapGlass.entrySet()) {
						...
						RP_SYS_WIP_SUMM2_Bean totBean = mapWipCluster.get(keyCluster);
						if (totBean == null) {
							totBean = new RP_SYS_WIP_SUMM2_Bean();
					...
					if (!Flag.TRUE) {
						 ...
						 * 확인출력
						 */
						for (Map.Entry<String, RP_SYS_WIP_SUMM2_Bean> entry : mapWipCluster.entrySet()) {
							Print.println("[%s] -> %s", entry.getKey(), entry.getValue());
						}
				...
				if (Flag.TRUE) {
					...
					for (RP_SYS_WIP_SUMM2_Bean outBean1 : outList1) {
						...
						RP_SYS_WIP_SUMM2_Bean bean = mapWipCluster.get(outBean1.getClusterId());
				...
				if (Flag.TRUE) {
					...
					Collections.sort(outList1, new Comparator<RP_SYS_WIP_SUMM2_Bean>() {
						...
						public int compare(RP_SYS_WIP_SUMM2_Bean bean1, RP_SYS_WIP_SUMM2_Bean bean2) {
							...
							// 2. GoodRatio DESC
							val1 = Double.parseDouble(bean1.getGoodRatio());
							val2 = Double.parseDouble(bean2.getGoodRatio());
				...
				if (Flag.TRUE) {
					...
					for (int i=outList1.size()-1; 0 <= i; i--) {
						double ratio = Double.parseDouble(outList1.get(i).getBadRatio());
						if (80.0 > ratio) {
							outList1.remove(i);
						}
					}
				...
				if (!Flag.TRUE) {
					/*
					 * TODO : 2014.11.27 : 최책임의 요청으로 위로직으로 변경함.
					 * RANK 10 이상 삭제
					 */
					for (int i=outList1.size()-1; 0 <= i; i--) {
	...
	public void afterMacroJob()
	{
		...
		if (Flag.TRUE) {
			try {
				...
				RP_SYS_WIP_SUMM2_Main.cntWList = outList1.size();
		...
		if (Flag.TRUE) {
			 ...
			 * 시간기록 끝
			 */
			RP_SYS_WIP_SUMM2_Main.runMSec = (System.nanoTime() - RP_SYS_WIP_SUMM2_Main.runMSec) / 1000 / 1000;
			
			if (Flag.TRUE) Logger.info("##### took time : %d ms", RP_SYS_WIP_SUMM2_Main.runMSec);
		}
		...
		if (Flag.TRUE) {
			...
			bean.setTable("ANAL_CSV");
			bean.setFields(new String[][] {
					{ "LIST_CNT   ", "" + RP_SYS_WIP_SUMM2_Main.cntWList },
					{ "E_TIME     ", "SYSDATE" },
					{ "R_MSEC     ", "" + RP_SYS_WIP_SUMM2_Main.runMSec },
