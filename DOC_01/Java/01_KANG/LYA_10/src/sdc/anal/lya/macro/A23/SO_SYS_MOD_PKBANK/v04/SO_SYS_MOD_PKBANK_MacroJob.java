...
public class SO_SYS_MOD_PKBANK_MacroJob extends AbstractMacroJob
{
	...
	public void beforeMacroJob()
	{
		...
		if (Flag.TRUE) {
			 ...
			 * 시간기록 시작
			 */
			SO_SYS_MOD_PKBANK_Main.runMSec = System.nanoTime();
		}
		...
		if (Flag.TRUE) {
			...
			bean.setWhere(String.format("JOB_ID = %s AND PROG_NM = %s", StrUtil.quote(jobId), StrUtil.quote(SO_SYS_MOD_PKBANK_Main.dsName)));
		...
		if (Flag.TRUE) {
			...
			bean.setFields(new String[][] {
					...
					{ "CMD_CODE   ", StrUtil.quote(SO_SYS_MOD_PKBANK_Main.dsName.substring(0, 3)) },
					{ "PROG_NM    ", StrUtil.quote(SO_SYS_MOD_PKBANK_Main.dsName) },
					{ "CSV_NM     ", StrUtil.quote(SO_SYS_MOD_PKBANK_Main.dsName + ".csv") },
	...
	public void afterMacroJob()
	{
		...
		if (Flag.TRUE) {
			try {
				...
				SO_SYS_MOD_PKBANK_Main.cntWList = outList1.size();
		...
		if (Flag.TRUE) {
			 ...
			 * 시간기록 끝
			 */
			SO_SYS_MOD_PKBANK_Main.runMSec = (System.nanoTime() - SO_SYS_MOD_PKBANK_Main.runMSec) / 1000 / 1000;
			
			if (Flag.TRUE) Logger.info("##### took time : %d ms", SO_SYS_MOD_PKBANK_Main.runMSec);
		}
		...
		if (Flag.TRUE) {
			...
			bean.setTable("ANAL_CSV");
			bean.setFields(new String[][] {
					{ "LIST_CNT   ", "" + SO_SYS_MOD_PKBANK_Main.cntWList },
					{ "E_TIME     ", "SYSDATE" },
					{ "R_MSEC     ", "" + SO_SYS_MOD_PKBANK_Main.runMSec },
