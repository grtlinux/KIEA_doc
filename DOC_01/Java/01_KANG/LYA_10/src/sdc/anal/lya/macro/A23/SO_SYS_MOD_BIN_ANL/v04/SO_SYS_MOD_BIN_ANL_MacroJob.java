...
public class SO_SYS_MOD_BIN_ANL_MacroJob extends AbstractMacroJob
{
	...
	public void beforeMacroJob()
	{
		...
		if (Flag.TRUE) {
			 ...
			 * 시간기록 시작
			 */
			SO_SYS_MOD_BIN_ANL_Main.runMSec = System.nanoTime();
		}
		...
		if (Flag.TRUE) {
			...
			bean.setWhere(String.format("JOB_ID = %s AND PROG_NM = %s", StrUtil.quote(jobId), StrUtil.quote(SO_SYS_MOD_BIN_ANL_Main.dsName)));
		...
		if (Flag.TRUE) {
			...
			bean.setFields(new String[][] {
					...
					{ "CMD_CODE   ", StrUtil.quote(SO_SYS_MOD_BIN_ANL_Main.dsName.substring(0, 3)) },
					{ "PROG_NM    ", StrUtil.quote(SO_SYS_MOD_BIN_ANL_Main.dsName) },
					{ "CSV_NM     ", StrUtil.quote(SO_SYS_MOD_BIN_ANL_Main.dsName + ".csv") },
			...
			try {
				...
				if (Flag.TRUE) {
					...
					for (SO_SYS_MOD_BIN_CP_Bean inBean1 : inList1) {
						...
						outBean1.setBadRate   ("" + (Double.parseDouble(inBean1.getBad()) * 100 / badSum));
						outBean1.setGoodRate  ("" + (Double.parseDouble(inBean1.getGood()) * 100 / goodSum));
	...
	public void afterMacroJob()
	{
		...
		if (Flag.TRUE) {
			try {
				...
				SO_SYS_MOD_BIN_ANL_Main.cntWList = outList1.size();
		...
		if (Flag.TRUE) {
			 ...
			 * 시간기록 끝
			 */
			SO_SYS_MOD_BIN_ANL_Main.runMSec = (System.nanoTime() - SO_SYS_MOD_BIN_ANL_Main.runMSec) / 1000 / 1000;
			
			if (Flag.TRUE) Logger.info("##### took time : %d ms", SO_SYS_MOD_BIN_ANL_Main.runMSec);
		}
		...
		if (Flag.TRUE) {
			...
			bean.setTable("ANAL_CSV");
			bean.setFields(new String[][] {
					{ "LIST_CNT   ", "" + SO_SYS_MOD_BIN_ANL_Main.cntWList },
					{ "E_TIME     ", "SYSDATE" },
					{ "R_MSEC     ", "" + SO_SYS_MOD_BIN_ANL_Main.runMSec },
