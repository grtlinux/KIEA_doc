...
import kiea.proj.sdc.anal.base.lya.detector.v03.ParamInfo;
import kiea.proj.sdc.anal.common.Connection;
import kiea.proj.sdc.anal.util.ServiceDetectorProperties;
import kiea.proj.sdc.anal.util.StrUtil;

public class CopyListMain
{
	...
	public CopyListMain()
	{
		if (Flag.TRUE) {
			...
			if (Flag.TRUE) Print.println("* [SvcCode:%s] [ROWNUM:%s]", this.svcCode, this.rowNum);
		}
	...
	private String getMaxQafJobId()
	{
		...
		if (Flag.TRUE) {
			try {
				...
				if (Flag.TRUE) System.out.println("MAX_JOBID [" + query + "]");
	...
	private List<String[]> getJobIdParams(String maxQJobId)
	{
		...
		if (Flag.TRUE) {
			try {
				...
				if (Flag.TRUE) System.out.println("LIST [" + query + "]");
