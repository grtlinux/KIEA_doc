...
public class GetListBeforeDetectMain
{
	...
	public GetListBeforeDetectMain()
	{
		if (Flag.TRUE) {
			...
			if (Flag.TRUE) Print.println("==================== DateTime : %s ====================", DateTime.getDateTime(2));
	...
	private List<String> getDetectJobIdList(String maxQafJobId)
	{
		...
		if (Flag.TRUE) {
			try {
				...
				if ("010".equals(this.svcCode)) {
				...
				} else if ("020".equals(this.svcCode)) {
					query = String.format("\n"
							+ "WITH TBL AS (                                         \n"
							+ "    SELECT                                            \n"
							+ "        job_id                                        \n"
					...
					System.out.println("ERROR : service code error");
					System.exit(0);
				}
	...
	{
		if (!Flag.TRUE) SystemProperties.setTesting("010");  // LYA
		if (!Flag.TRUE) SystemProperties.setTesting("020");  // MURA
		if (!Flag.TRUE) SystemProperties.getInstance().printList();
		
		if (Flag.TRUE) {
			new GetListBeforeDetectMain().execute();
		}
	}
