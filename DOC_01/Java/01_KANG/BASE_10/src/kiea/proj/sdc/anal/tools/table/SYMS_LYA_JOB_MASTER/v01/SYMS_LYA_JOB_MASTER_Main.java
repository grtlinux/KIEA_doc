package kiea.proj.sdc.anal.tools.table.SYMS_LYA_JOB_MASTER.v01;
...
public class SYMS_LYA_JOB_MASTER_Main
{
	private OracleConnection conn = null;
	private Statement stmt = null;

	private SYMS_LYA_JOB_MASTER_Main()
	{
		if (Flag.TRUE) {
			try {
			...
			try {
				...
				if (Flag.TRUE) {
					...
					for (int i=0; i < 50 && resultSet.next(); i++) {
						...
						String analReqStatus = resultSet.getString("ANAL_REQ_STATUS");
						String createDate = resultSet.getString("CREATE_DATE");
						
						if (Flag.TRUE) Print.println("%4d) [%s] [%s] [%s] [%s]", i, jobId, createDate, analReqStatus, jobTitle);
					}
				}
	...
	private static SYMS_LYA_JOB_MASTER_Main instance = null;
	
	public static SYMS_LYA_JOB_MASTER_Main getInstance()
	{
		if (instance == null) {
			instance = new SYMS_LYA_JOB_MASTER_Main();
		}
