...
public class ANAL_PARAM_Main
{
			...
			try {
				...
				if (Flag.TRUE) {
					...
					for (int i=0; i < 1000 && resultSet.next(); i++) {
						...
						if (Flag.TRUE) Print.println("%4d) [%s] [%s] [%s] [%s]", i, jobId, seq, dipName, dipValue);
					}
				}
	...
	public boolean delete_01()
	{
		...
		if (Flag.TRUE) {
			try {
				String query = String.format("\n"
						+ " DELETE FROM anal_param WHERE 1=1 \n"
			...
			try {
				...
				if (Flag.TRUE) {
					...
					for (int i=0; i < 10000 && resultSet.next(); i++) {
						String jobId = resultSet.getString("JOB_ID");
						String seq = resultSet.getString("SEQ");
						String paramName = resultSet.getString("PARAM_NM");
						String paramValue = resultSet.getString("PARAM_VAL");
						...
						if (Flag.TRUE) Print.println("%4d) [%16s] [%s] [%s] [%s]", i, jobId, seq, paramName, paramValue);
					}
				}
