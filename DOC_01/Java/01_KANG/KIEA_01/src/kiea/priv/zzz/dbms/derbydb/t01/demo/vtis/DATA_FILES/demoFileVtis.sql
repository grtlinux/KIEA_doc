...
--
create function svnLogReader( logFileName varchar( 32672 ) ) returns TABLE
(
	 XID            varchar( 15 ),
...
--
create function propertyFileVTI( fileName varchar( 32672 ) ) returns TABLE
(
...
select
	...
	, count(*) as commits
from
	table( svnLogReader( 'svn_log.txt' ) ) s
where
	commit_time between timestamp( '2006-01-01 00:00:00' ) and timestamp( '2007-01-01 00:00:00' )
group by committer
;
...
from
	table( propertyFileVTI( 'messages_en.properties' ) ) m_english
where
	...
	(
		select
			m_french.messageID
		from
			table( propertyFileVTI( 'messages_fr.properties' ) ) m_french
	)
;
...
select
	s.*
from
	table( apacheNaturalLogFile( 'ApacheServerLog.xml' ) ) s
;
...
select
	s.*
from
	table( apacheNaturalJiraReport( 'DerbyJiraReport.xml' ) ) s
where
